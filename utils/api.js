...
// Alternative Allocation Settings API (Bitcoin & Gold)
export const fetchAlternativeAllocationSettings = async () => {
  try {
    const res = await fetch(`${API_BASE}/alternative-allocation-settings`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' }
    });
    const data = await handleResponse(res);
    console.log('Fetched alternative allocation settings:', data);
    return data;
  } catch (error) {
    console.error('Error fetching alternative allocation settings:', error);
    throw error;
  }
};

export const updateAlternativeAllocationSettings = async (bitcoinAllocation, goldAllocation) => {
  try {
    const res = await fetch(`${API_BASE}/alternative-allocation-settings`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    const data = await handleResponse(res);
    console.log('Updated alternative allocation settings:', data);
    return data;
  } catch (error) {
    console.error('Error updating alternative allocation settings:', error);
    throw error;
  }
};
