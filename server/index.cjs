...
app.post('/api/alternative-investments', (req, res) => {
  try {
    const { name, asset_type, platform } = req.body;
    const stmt = db.prepare(`
      INSERT INTO alternative_investments (name, asset_type, platform)
      VALUES (?, ?, ?)
    `);
    const result = stmt.run(name, asset_type, platform);
    res.json({ id: result.lastInsertRowid, name, asset_type, platform });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
// GET alternative allocation settings (Bitcoin and Gold)
app.get('/api/alternative-allocation-settings', (req, res) => {
  try {
    const settings = getAlternativeAllocationSettings();
    res.json(settings);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.put('/api/alternative-allocation-settings', (req, res) => {
  try {
    const { bitcoinAllocation, goldAllocation } = req.body;
    
    if (bitcoinAllocation === undefined || goldAllocation === undefined) {
      return res.status(400).json({ error: 'Missing required fields (bitcoinAllocation, goldAllocation)' });
    }
    
    if (bitcoinAllocation + goldAllocation !== 100) {
      return res.status(400).json({ error: 'Bitcoin and Gold allocations must total 100%' });
    }
    
    const settings = updateAlternativeAllocationSettings(bitcoinAllocation, goldAllocation);
    res.json(settings);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
