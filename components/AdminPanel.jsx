...
export default function AdminPanel({
  ...
  // Note: alternativeAllocationSettings and setAlternativeAllocationSettings are passed as props from parent
  ...
  // Load alternative allocation settings (Bitcoin and Gold percentages) from database
  useEffect(() => {
      ...
      {activeAdminView === 'allocation-detail' && (
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-6 border border-orange-200 shadow-sm hover:shadow-md transition">
              ...
              <div className="bg-white rounded-lg p-4 mb-4 border border-orange-200">
                ...
                <div className="flex gap-2 items-center">
                  <input 
                    ...
                    min="0" 
                    max="100" 
                    value={editingAllocation ? tempAllocationPercentages.bitcoinAllocation : alternativeAllocationSettings.bitcoinAllocation}
                    onChange={(e) => {
              ...
              <div className="bg-orange-100 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-orange-600">
                  RM {(((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100)) * ((editingAllocation ? tempAllocationPercentages.bitcoinAllocation : alternativeAllocationSettings.bitcoinAllocation) / 100)).toLocaleString('en-MY', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                </p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-6 border border-yellow-200 shadow-sm hover:shadow-md transition">
              ...
              <div className="bg-white rounded-lg p-4 mb-4 border border-yellow-200">
                ...
                <div className="flex gap-2 items-center">
                  <input 
                    ...
                    min="0" 
                    max="100" 
                    value={editingAllocation ? tempAllocationPercentages.goldAllocation : alternativeAllocationSettings.goldAllocation}
                    onChange={(e) => {
                      if (editingAllocation) {
                        const val = Math.max(0, Math.min(100, Number(e.target.value)));
              ...
              <div className="bg-yellow-100 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-yellow-600">
                  RM {(((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100)) * ((editingAllocation ? tempAllocationPercentages.goldAllocation : alternativeAllocationSettings.goldAllocation) / 100)).toLocaleString('en-MY', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                </p>
              </div>
            </div>
          </div>
          ...
          <button
            onClick={async () => {
              if (!editingAllocation) {
                setTempAllocationPercentages({...tempAllocationPercentages, bitcoinAllocation: alternativeAllocationSettings.bitcoinAllocation, goldAllocation: alternativeAllocationSettings.goldAllocation});
                ...
                // Validate that Bitcoin and Gold percentages sum to 100
                const totalAllocation = parseFloat(tempAllocationPercentages.bitcoinAllocation || 0) + parseFloat(tempAllocationPercentages.goldAllocation || 0);
                if (Math.abs(totalAllocation - 100) > 0.01) {
                  alert('Bitcoin and Gold allocations must total 100%');
                  return;
                }
                
                // Update Bitcoin and Gold allocations via new API
                try {
                  const result = await updateAlternativeAllocationSettings(
                    parseFloat(tempAllocationPercentages.bitcoinAllocation),
                    parseFloat(tempAllocationPercentages.goldAllocation)
                  );
                  console.log('Alternative allocations updated successfully:', result);
                  
                  // Update the local state with the new values
                  setAlternativeAllocationSettings({
                    bitcoinAllocation: result.bitcoin_allocation,
                    goldAllocation: result.gold_allocation
                  });
                } catch (e) {
                  console.error('Error updating alternative allocations:', e);
                  updateSuccess = false;
                }
