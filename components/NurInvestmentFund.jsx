...
const NurInvestmentFund = () => {
  ...
  // Alternative allocation settings (Bitcoin and Gold percentages)
  const [alternativeAllocationSettings, setAlternativeAllocationSettings] = useState({
    bitcoinAllocation: 50,
    goldAllocation: 50
  });
  ...
  // Load alternative allocation settings (Bitcoin and Gold percentages)
  useEffect(() => {
    const loadAlternativeAllocationSettings = async () => {
      try {
        const settings = await fetchAlternativeAllocationSettings();
        setAlternativeAllocationSettings({
          bitcoinAllocation: settings.bitcoin_allocation,
          goldAllocation: settings.gold_allocation
        });
      } catch (error) {
        console.error('Failed to load alternative allocation settings:', error);
      }
    };
  ...
  const AlternativeInvestmentListView = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Alternative Investments</h2>
            <p className="text-gray-600 mt-2">RM {totalValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            <p className="text-sm text-gray-500">{alternativeInvestments.length} assets • Total market value</p>
          </div>

          <div className="mb-8">
            <h3 className="text-lg font-semibold mb-2">Fund Allocation Strategy</h3>
            <p className="text-sm text-gray-600 mb-4">
              Total Alternatives Allocation Value: <span className="font-bold text-blue-600">RM {((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </p>
              ...
              <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-6 border border-orange-200 shadow-sm hover:shadow-md transition">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-600 mt-1">Cryptocurrency holdings</p>
                  </div>
                  <span className="text-2xl">₿</span>
                </div>
                <div className="mt-4 pt-4 border-t border-orange-200">
                  <p className="text-3xl font-bold text-orange-600">{alternativeAllocationSettings.bitcoinAllocation}%</p>
                  <p className="text-xs text-gray-600 mt-2">Target allocation</p>
                  <p className="text-sm font-semibold text-orange-700 mt-2">Should invest: RM {((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.bitcoinAllocation / 100)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  <p className="text-sm font-semibold text-orange-700 mt-4">Market value: RM {alternativeInvestments.filter(a => a.asset_type === 'crypto' || a.asset_type === 'Bitcoin').reduce((sum, a) => sum + (a.current_value || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  {(() => {
                    const shouldInvest = ((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.bitcoinAllocation / 100));
                    const marketValue = alternativeInvestments.filter(a => a.asset_type === 'crypto' || a.asset_type === 'Bitcoin').reduce((sum, a) => sum + (a.current_value || 0), 0);
                  ...
                  <div className="mt-3">
                    ...
                    <div className="w-full bg-orange-200 rounded-full h-2">
                      <div className="bg-orange-600 h-2 rounded-full" style={{width: `${Math.min(100, (alternativeInvestments.filter(a => a.asset_type === 'crypto' || a.asset_type === 'Bitcoin').reduce((sum, a) => sum + (a.current_value || 0), 0) / (((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.bitcoinAllocation / 100)) || 1)) * 100)}%`}}></div>
                    </div>
                    <p className="text-xs text-orange-700 font-semibold mt-1">{Math.min(100, (alternativeInvestments.filter(a => a.asset_type === 'crypto' || a.asset_type === 'Bitcoin').reduce((sum, a) => sum + (a.current_value || 0), 0) / (((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.bitcoinAllocation / 100)) || 1)) * 100).toFixed(1)}%</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-6 border border-yellow-200 shadow-sm hover:shadow-md transition">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-600 mt-1">Precious metal holdings</p>
                  </div>
                  <span className="text-2xl">🏆</span>
                </div>
                <div className="mt-4 pt-4 border-t border-yellow-200">
                  <p className="text-3xl font-bold text-yellow-600">{alternativeAllocationSettings.goldAllocation}%</p>
                  <p className="text-xs text-gray-600 mt-2">Target allocation</p>
                  <p className="text-sm font-semibold text-yellow-700 mt-2">Should invest: RM {((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.goldAllocation / 100)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  <p className="text-sm font-semibold text-yellow-700 mt-4">Market value: RM {alternativeInvestments.filter(a => a.asset_type === 'gold' || a.asset_type === 'Gold').reduce((sum, a) => sum + (a.current_value || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  {(() => {
                    const shouldInvest = ((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.goldAllocation / 100));
                    const marketValue = alternativeInvestments.filter(a => a.asset_type === 'gold' || a.asset_type === 'Gold').reduce((sum, a) => sum + (a.current_value || 0), 0);
                    const difference = shouldInvest - marketValue;
                    const percentDiff = (difference / (shouldInvest || 1)) * 100;
                    
                    if (difference > 0) {
                      return <p className="text-sm font-semibold text-yellow-700 mt-3 bg-yellow-50 p-2 rounded">🟡 BUY: RM {difference.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>;
                    } else if (percentDiff < -10) {
                      return <p className="text-sm font-semibold text-yellow-700 mt-3 bg-yellow-50 p-2 rounded">🔴 SELL: RM {Math.abs(difference).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} to rebalance</p>;
                    }
                    return null;
                  })()}
                  <div className="mt-3">
                    ...
                    <div className="w-full bg-yellow-200 rounded-full h-2">
                      <div className="bg-yellow-600 h-2 rounded-full" style={{width: `${Math.min(100, (alternativeInvestments.filter(a => a.asset_type === 'gold' || a.asset_type === 'Gold').reduce((sum, a) => sum + (a.current_value || 0), 0) / (((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.goldAllocation / 100)) || 1)) * 100)}%`}}></div>
                    </div>
                    <p className="text-xs text-yellow-700 font-semibold mt-1">{Math.min(100, (alternativeInvestments.filter(a => a.asset_type === 'gold' || a.asset_type === 'Gold').reduce((sum, a) => sum + (a.current_value || 0), 0) / (((funds[0]?.target_value || 0) * (allocationPercentages.alternatives / 100) * (alternativeAllocationSettings.goldAllocation / 100)) || 1)) * 100).toFixed(1)}%</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Investments ({alternativeInvestments.length})</h3>
              <input 
                type="text" 
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="text-right py-3 px-4 font-semibold">Units Held</th>
                      <th className="text-right py-3 px-4 font-semibold">Unit Price (RM)</th>
                  ...
                  <tbody>
                    {alternativeInvestments.map((inv) => {
                      const profit = (inv.current_value || 0) - (inv.cost || 0);
                      const returnPercent = (inv.cost || 0) > 0 ? ((profit / inv.cost) * 100) : 0;
                      const unitPrice = inv.quantity > 0 ? inv.current_value / inv.quantity : 0;
                      
                      return (
                        <tr key={inv.id} className="border-t hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedAlternativeInvestment(inv.id)}>
                          <td className="py-3 px-4 text-sm capitalize font-medium">{inv.asset_type}</td>
                          <td className="py-3 px-4 text-sm">
                            <a className="text-blue-600 hover:underline">{inv.name}</a>
                          </td>
                          <td className="py-3 px-4 text-right text-sm">{inv.quantity}</td>
                          <td className="py-3 px-4 text-right text-sm">RM {unitPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                          <td className="py-3 px-4 text-right text-sm font-semibold">RM {(inv.current_value || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                          <td className="py-3 px-4 text-right text-sm">RM {(inv.cost || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                          <td className="py-3 px-4 text-right text-sm" style={{ color: profit >= 0 ? '#10b981' : '#ef4444' }}>
                            {profit >= 0 ? '+' : ''}RM {profit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td className="py-3 px-4 text-right text-sm" style={{ color: returnPercent >= 0 ? '#10b981' : '#ef4444' }}>
          ...
          <div className="bg-white rounded-lg border p-6">
            <h3 className="text-lg font-semibold mb-4">Historic Investments - Alternative Investments (2025)</h3>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={[{ year: '2025', value: calculateTotalAlternativeInvestmentsValue() / 5.2 }]}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="year" />
                <YAxis />
                <Tooltip 
                  formatter={(value) => {
                    const formatted = (value * 5.2).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                    return `RM ${formatted}`;
                  }}
                  labelFormatter={(label) => `Year: ${label}`}
                />
      ...
      case 'admin':
        return (
          <AdminPanel
            ...
            alternativeAllocationSettings={alternativeAllocationSettings}
            setAlternativeAllocationSettings={setAlternativeAllocationSettings}
            funds={funds}
            setFunds={setFunds}
          />
        );
